...
# A per-tick duration held in SECONDS, found 2026-09-07 by reading .lit4 - the
# float constant pool, which no scan in this project had ever looked at. ee-gcc
# materialises 30.0 with `lui` but pools 1/30, so every scan for immediates was
# structurally blind to this class. FUN_0017D8C0 is an effect-node class entry
# (function pointer at 002C3E80) whose body runs [node+0x4E4] down by 1/30 per
# tick and expires it at zero - identical step and identical 9 ticks at both
# rates, so it expires in half the real time at 60fps.
#
# The fix is one word and needs no trampoline: repoint the `lwc1 ...($gp)` at a
# 1/60 the pool already holds. This is the first fix in the project of that
# shape, and the same move is available at the other 22 pooled-rate sites.
FLASHDUR = ["60FPS - blast flash duration"]
...
ENABLED_IN_INI = (SHIPPED + AIRBORNE + EFFECTS + TWEENS + PARTICLES
                  ...
                  + BLASTDUR + SEQWAIT + PHASE + FLASHDUR + SPARES)
...
PRESETS = {
    ...
    "nophase": (SHIPPED + AIRBORNE + EFFECTS + TWEENS + PARTICLES + HOVER
                + BLAST + BLASTDUR + SEQWAIT + FLASHDUR),
    "noflash": (SHIPPED + AIRBORNE + EFFECTS + TWEENS + PARTICLES + HOVER
                + BLAST + BLASTDUR + SEQWAIT + PHASE),
    "full": (SHIPPED + AIRBORNE + EFFECTS + TWEENS + PARTICLES + HOVER
             + BLAST + BLASTDUR + SEQWAIT + PHASE + FLASHDUR),
}
