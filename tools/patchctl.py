...
# frames, so at 60fps each one expires in half its real time. 21 of the 28 sites
...
# count of frames, and it is the only one of the 22 that is - it sits inside
# FUN_001E6DC8, which IS state 157. Gating an index makes that state walk its
# table at half rate and, because the advance is conditional, skip it entirely
# on an odd tick. It has been removed from the group. The trap itself was never
# reproduced under control, so this wants a play test before a release is cut.
...
PRESETS = {
    ...
    # PHASE was WITHDRAWN 2026-09-07 and reinstated 2026-09-16. It gated 22
    # counters inside the fighter state machine - the exact mechanism behind a
    # fighter that fails to leave a state - and every one of them was scored
    # against two GROUNDED oracles, a held charge and a mashed rush. No airborne
    # state was ever tested, and the user hit a repeatable trap in state 157
    # (FUN_001E6DC8) with no pending transition.
    #
    # The hole in that validation has since been closed by reading rather than
    # by play: of the 22, exactly one - 001E6F40 - is an index into a table
    # instead of a count of frames, and it is inside FUN_001E6DC8 itself. It is
    # out of the group, and PHASE now gates 21 clocks.
    ...
    "full": (SHIPPED + AIRBORNE + EFFECTS + TWEENS + PARTICLES + HOVER
             ...
             + BEAMCLASH + PHASE + STAGE + SOLARFLARE + THROWN + SCRIPTCLOCK),
