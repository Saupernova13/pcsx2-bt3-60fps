...
PCSX2 fork, not yet public) under `tools/pcsxroo/` - a checkout of pcsxroo next
to this one is required for the tools here to run. PCSXROO is a PCSX2 fork with
a debug server (memory, breakpoints, watchpoints, save states, pad injection,
screenshots) on port 28110; it is what the PCSXROO-transport tools drive. It
runs in portable mode, so its `cheats/`, `sstates/` and `snaps/` sit next to
the executable:

    pcsxroo-qt.exe -debugserver 28110 -- "path\to\bt3.cso"

Paths are discovered automatically. Override anything by creating `local.json`
in this repo's root:
